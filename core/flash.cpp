...
{
    if(!write_to_snapshot(snapshot, &nand, sizeof(nand)))
        return false;
    ...
    for(unsigned int cur_modified_block_nr = 0; cur_modified_block_nr < num_blocks; ++cur_modified_block_nr)
    {
        ...
        if(!write_to_snapshot(snapshot, nand_data + block_size * cur_modified_block_nr, block_size))
            return true;
    }
...
bool flash_resume(const emu_snapshot *snapshot)
{
    ...
    if(!read_from_snapshot(snapshot, &nand, sizeof(nand)))
        return false;
    ...
    for(unsigned int cur_modified_block_nr = 0; cur_modified_block_nr < num_blocks; ++cur_modified_block_nr)
    {
        ...
        if(!read_from_snapshot(snapshot, nand_data + block_size * cur_modified_block_nr, block_size))
            return false;
    }
