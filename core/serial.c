...
{
    return read_from_snapshot(snapshot, &serial, sizeof(serial));
}

bool serial_suspend(emu_snapshot *snapshot)
{
    return write_to_snapshot(snapshot, &serial, sizeof(serial));
}
...
{
    return read_from_snapshot(snapshot, &serial_cx, sizeof(serial_cx));
}

bool serial_cx_suspend(emu_snapshot *snapshot)
{
    return write_to_snapshot(snapshot, &serial_cx, sizeof(serial_cx));
}
