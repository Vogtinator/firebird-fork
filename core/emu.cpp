...
bool read_from_snapshot(const emu_snapshot *snapshot, void *dest, int size)
{
    return gzread((gzFile)snapshot->stream_handle, dest, size) == size;
}

bool write_to_snapshot(emu_snapshot *snapshot, const void *src, int size)
{
...
bool emu_start(unsigned int port_gdb, unsigned int port_rdbg, const char *snapshot_file)
{
    ...
    if(snapshot_file)
    {
        ...
        // Read the header
        if(!read_from_snapshot(&snapshot, &snapshot.header, sizeof(snapshot.header)))
        {
            gzclose(gzf);
            return false;
        }
...
bool emu_suspend(const char *file)
{
    ...
    if(!write_to_snapshot(&snapshot, &snapshot.header, sizeof(snapshot.header))
